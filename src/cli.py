...
@click.option("--out", default=None, help="Output JSONL file (default data/raw/youtube_comments.jsonl)")
@click.option("--max-comments", type=int, default=None)
def collect_youtube(video_url, out, max_comments):
    """Scrape YouTube comments using Selenium"""
...
def collect_rss(rss_url, out):
    """Parse RSS feed and store results in JSONL"""
    os.makedirs(RAW_DIR, exist_ok=True)
...
@click.option("--out-dir", required=True, help="Directory for reports & sentiment output")
@click.option("--use-hf", is_flag=True, help="Use HuggingFace sentiment model (heavy)")
def analyze(storage, out_dir, use_hf):
    """Run sentiment analysis + keyword analysis + topics + wordcloud"""
...
@click.option("--sentiment-jsonl", required=True, help="Path to sentiment JSONL")
@click.option("--out-html", required=True)
@click.option("--top-global", required=False)
@click.option("--per-source-top", required=False)
@click.option("--topics", required=False)
def generate_report(sentiment_jsonl, out_html, top_global, per_source_top, topics):
    """Generate full HTML analytics report"""

    print("Generating report...")

    path = generate_html_report(
        sentiment_jsonl_path=sentiment_jsonl,
        out_html=out_html,
        top_global_json=top_global,
        per_source_top_json=per_source_top,
        topics_json=topics,
        title="TextInsights — Analysis Report"
    )
